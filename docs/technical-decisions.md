...
## 3.1 複数話者・声プロファイル関連の決定事項（確定）

タスク 5 完了後、以下の技術決定を行った。

| 項目 | 選択肢 | 決定 | 理由 |
|------|--------|------|------|
| 声プロファイルの管理方式 | A: 話者別ディレクトリ構造 / B: フラット構造 + メタデータファイル | **B** | シンプルで管理しやすい。metadata.csv/JSON で一元管理。 |
| メタデータ形式 | A: CSV / B: JSON | **A（CSV 優先）** | 手動編集が容易。Python 標準ライブラリで読み込み可能。大規模化時は JSON に移行検討。 |
| テスト合成 UI | A: CLI ツール / B: WebUI (Gradio 等) / C: Discord Bot 内 | **A** | MVP 最小実装。機能確認が目的。将来的に B/C 追加可能。 |
| モデル永続化 | A: 声プロファイル（ref_audio + ref_text）/ B: ファインチューニング済み .pth | **A** | Qwen3-TTS はゼロショット設計。VRAM とデータ量の制約から、現時点ではファインチューニング不採用。 |
| Discord 実装時のレイテンシ対策 | A: 声プロファイル + キャッシュ / B: ファインチューニング | **A（フェーズ1）** | モデルと参照音声をメモリキャッシュして高速化。レイテンシが問題になればフェーズ2で B を検討。 |

### ファインチューニングについて

- **現時点では非採用**: VRAM 12GB < 22GB 必要、学習時間長、効果不明確。
- **Discord 実装後**: 実測レイテンシが許容範囲外の場合に再検討する。
- **将来の選択肢として残す**: レイテンシ・品質要件が変わった場合に検討する。
